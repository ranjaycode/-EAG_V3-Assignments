...
DEFAULT_URL = os.getenv("LLM_GATEWAY_V8_URL", "http://127.0.0.1:8108")
    ...
    def chat(self, prompt: str = None, *,
        ...
        for attempt in range(5):
            try:
                ...
                if attempt == 4 or e.response.status_code != 503:
                    raise
                print(f"[gateway client] chat failed with 503 (attempt {attempt + 1}/5). Retrying in 4.5s...")
                time.sleep(4.5)
            except Exception as e:
                if attempt == 4:
                    raise
                print(f"[gateway client] chat failed: {e} (attempt {attempt + 1}/5). Retrying in 4.5s...")
                time.sleep(4.5)
    ...
    def chat_batch(self, calls: list[dict], max_concurrency: int = 4) -> list[dict]:
        ...
        for attempt in range(5):
            try:
                ...
                if attempt == 4 or e.response.status_code != 503:
                    raise
                print(f"[gateway client] chat_batch failed with 503 (attempt {attempt + 1}/5). Retrying in 4.5s...")
                time.sleep(4.5)
            except Exception as e:
                if attempt == 4:
                    raise
                print(f"[gateway client] chat_batch failed: {e} (attempt {attempt + 1}/5). Retrying in 4.5s...")
                time.sleep(4.5)
    ...
    def embed(self, text: str,
        ...
        for attempt in range(5):
            try:
                ...
                if attempt == 4 or e.response.status_code != 503:
                    raise
                print(f"[gateway client] embed failed with 503 (attempt {attempt + 1}/5). Retrying in 2.5s...")
                time.sleep(2.5)
            except Exception as e:
                if attempt == 4:
                    raise
                print(f"[gateway client] embed failed: {e} (attempt {attempt + 1}/5). Retrying in 2.5s...")
                time.sleep(2.5)
