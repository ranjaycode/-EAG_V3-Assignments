...
_TOOL_CATALOG = {
    ...
    "browser_navigate": {
        ...
        "description": "Navigate to a URL using the automated browser and capture a screenshot.",
        "input_schema": {
            "type": "object",
            "properties": {
                "url": {"type": "string"}
    ...
    "browser_click": {
        "name": "browser_click",
        "description": "Click an element matching the CSS or text selector and capture a screenshot.",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    "browser_type": {
        "name": "browser_type",
        "description": "Type text into an input element matching the selector.",
        "input_schema": {
            "type": "object",
            "properties": {
                "selector": {"type": "string"},
    ...
    "browser_get_state": {
        ...
        "description": "Get the current page URL, title, content preview, and a list of visible elements.",
        "input_schema": {
            "type": "object",
            "properties": {}
        }
    ...
    "browser_log_path": {
        "name": "browser_log_path",
        "description": "Log the chosen browser path ('extract', 'deterministic', 'a11y', 'vision', 'blocked').",
        "input_schema": {
            "type": "object",
            "properties": {
                "path_chosen": {"type": "string"}
            },
            "required": ["path_chosen"]
        }
    },
    "computer_hotkey": {
        "name": "computer_hotkey",
        "description": "Press a key combination or keyboard shortcut. Example: computer_hotkey(['ctrl', 'c']).",
        "input_schema": {
            "type": "object",
            "properties": {
                "keys": {
    ...
    "computer_type": {
        "name": "computer_type",
        "description": "Type a string of characters using the keyboard. Example: computer_type('15 * 6 =').",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    "computer_click": {
        "name": "computer_click",
        "description": "Click at the specified screen coordinates (x, y). Example: computer_click(450, 300).",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    "computer_get_state": {
        ...
        "description": "Get the current state of the computer, including screen size, active window title, and mouse position.",
        "input_schema": {
            "type": "object",
            "properties": {}
        }
    ...
    "computer_launch_app": {
        ...
        "description": "Launch an OS application. Example: computer_launch_app('calc.exe').",
        "input_schema": {
            "type": "object",
            "properties": {
                "app_path": {"type": "string", "description": "Path to the app executable, e.g. 'calc.exe'"},
                "args": {
                    "type": "array",
                    "items": {"type": "string"},
                    "default": [],
    ...
    "electron_connect": {
        ...
        "description": "Connect to an Electron app (like VS Code) running with remote debugging port over CDP.",
        "input_schema": {
            "type": "object",
            "properties": {
                "port": {"type": "integer", "default": 9222, "description": "CDP remote debugging port"}
            }
        }
    },
    "browser_get_accessibility_tree": {
        "name": "browser_get_accessibility_tree",
        "description": "Get the accessibility tree (AX tree) of the current browser page.",
        "input_schema": {
            "type": "object",
            "properties": {}
        }
...
async def run_skill(skill: Skill, node_id: str, graph_nodes,
    ...
    print(f"DEBUG [{skill.name}]: text={reply.get('text')!r}")
    print(f"DEBUG [{skill.name}]: parsed={parsed!r}")
