...
MAX_TOOL_HOPS = 25  # headroom for multi-step browser interaction
...
async def run_with_tools(*, prompt: str, tools_payload: list[dict],
    ...
    server_params = StdioServerParameters(command=sys.executable, args=[str(MCP_SERVER)], env=env)
    try:
            ...
            async with ClientSession(read, write) as mcp:
                ...
                for _ in range(MAX_TOOL_HOPS + 1):
                    ...
                    for tc in tool_calls:
                        print(f"DEBUG [mcp_runner]: Calling tool {tc['name']} with args {tc.get('arguments')}")
                        result_text = await _dispatch_tool(mcp, tc["name"],
                                                          tc.get("arguments") or {})
                        print(f"DEBUG [mcp_runner]: Tool result: {result_text[:200]}...")
                        messages.append({
                            "role": "tool",
        ...
        import traceback
        print(f"DEBUG [mcp_runner] Exception in run_with_tools: {type(e).__name__}: {e}")
        traceback.print_exc()
        raise e
