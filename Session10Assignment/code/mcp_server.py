...
async def _save_desktop_screenshot_and_log(action_type: str, details: str):
    global _screenshots_count, _browser_actions
    session_dir = _get_current_session_dir()
    browser_dir = session_dir / "browser"
    browser_dir.mkdir(parents=True, exist_ok=True)
...
@mcp.tool()
async def computer_hotkey(keys: list[str]) -> dict:
    """Press a key combination or keyboard shortcut. Example: computer_hotkey(["ctrl", "c"])."""
    import pyautogui
    try:
        pyautogui.hotkey(*keys)
        # Wait a tiny bit for the UI to register the keypress
        import asyncio
        await asyncio.sleep(0.5)
        screenshot = await _save_desktop_screenshot_and_log("computer_hotkey", f"Pressed hotkey: {keys}")
        return {"status": "success", "screenshot": screenshot}
    except Exception as e:
        return {"status": "error", "message": str(e)}

@mcp.tool()
async def computer_type(text: str) -> dict:
    """Type a string of characters using the keyboard. Example: computer_type("15 * 6 =")."""
    import pyautogui
    try:
        pyautogui.write(text, interval=0.05)
        import asyncio
        await asyncio.sleep(0.5)
        screenshot = await _save_desktop_screenshot_and_log("computer_type", f"Typed text: {text}")
        return {"status": "success", "screenshot": screenshot}
    except Exception as e:
        return {"status": "error", "message": str(e)}

@mcp.tool()
async def computer_click(x: int, y: int) -> dict:
    """Click at the specified screen coordinates (x, y). Example: computer_click(450, 300)."""
    import pyautogui
    try:
        pyautogui.click(x, y)
        import asyncio
        await asyncio.sleep(0.5)
        screenshot = await _save_desktop_screenshot_and_log("computer_click", f"Clicked at ({x}, {y})")
        return {"status": "success", "screenshot": screenshot}
    except Exception as e:
        return {"status": "error", "message": str(e)}
...
async def computer_screenshot() -> dict:
    """Capture a screenshot of the entire desktop screen. Example: computer_screenshot()."""
    try:
        screenshot = await _save_desktop_screenshot_and_log("computer_screenshot", "Captured desktop screenshot")
        return {"status": "success", "screenshot": screenshot}
    except Exception as e:
        return {"status": "error", "message": str(e)}

@mcp.tool()
async def computer_get_state() -> dict:
    """Get the current state of the computer, including screen size, active window title, and mouse position. Example: computer_get_state()."""
    import pyautogui
    try:
...
@mcp.tool()
async def computer_launch_app(app_path: str, args: list[str] = []) -> dict:
    """Launch an OS application. Example: computer_launch_app("calc.exe")."""
    import subprocess
    try:
        creationflags = 0
        app_name = app_path.split("\\")[-1].split("/")[-1].lower()
        if "cmd" in app_name or "powershell" in app_name or "pwsh" in app_name:
        ...
        screenshot = await _save_desktop_screenshot_and_log("computer_launch_app", f"Launched app: {app_path} with args: {args}")
        return {"status": "success", "screenshot": screenshot}
    except Exception as e:
        return {"status": "error", "message": str(e)}
...
@mcp.tool()
async def computer_read_clipboard() -> dict:
    """Read the current text content of the clipboard. Example: computer_read_clipboard()."""
    import pyperclip
    try:
...
@mcp.tool()
async def electron_connect(port: int = 9222) -> dict:
    """Connect to an Electron app (like VS Code) running with remote debugging port over CDP. 
    Once connected, subsequent browser_navigate, browser_click, browser_type, browser_get_state, 
    and browser_screenshot tools will interact with the Electron app instead of the standard browser.
    """
    global _playwright, _browser, _page
    from playwright.async_api import async_playwright
    import asyncio
    ...
    ports_to_try = [port]
    if port == 9222:
        ports_to_try.append(9223)
    elif port == 9223:
        ports_to_try.append(9222)
        
    errors = []
    
    # Helper to check if context contains VS Code
    async def find_vscode_page(browser_obj):
        for ctx in browser_obj.contexts:
            for p in ctx.pages:
                try:
                    title = await p.title()
                    # Check for VS Code layout elements or title clues
                    is_vscode = None
                    try:
                        is_vscode = (await p.query_selector(".monaco-workbench")) or (await p.query_selector(".editor-container"))
                    except Exception:
                        pass
                    if is_vscode:
                        return p
                    if title and ("Visual Studio Code" in title or "usage.json" in title or p.url.startswith("vscode-file:")):
                        return p
                except Exception:
                    pass
        return None

    # Step 1: Try already running instances on specified ports
    for current_port in ports_to_try:
        try:
            print(f"[electron_connect] Attempting to connect to port {current_port}...")
            ...
            _browser = await _playwright.chromium.connect_over_cdp(f"http://localhost:{current_port}")
            
            # Polling for VS Code page
            for attempt in range(5):
                target_page = await find_vscode_page(_browser)
                if target_page:
                    _page = target_page
                    screenshot = await _save_screenshot_and_log("electron_connect", f"Connected to Electron app page: {await _page.title()} on port {current_port}")
                    return {
                        "status": "success",
                        "title": await _page.title(),
                        "url": _page.url,
                        "screenshot": screenshot,
                        "connected_port": current_port
                    }
                await asyncio.sleep(0.5)
            
            errors.append(f"Connected to CDP on port {current_port}, but no target VS Code page was found.")
        except Exception as e:
            errors.append(f"Failed to connect over CDP to port {current_port}: {e}")
            
    # Step 2: If we couldn't connect, launch a new VS Code instance on port 9223!
    try:
        print("[electron_connect] VS Code not found. Launching a new instance on port 9223...")
        parent_dir = Path(__file__).resolve().parent
        user_data = str(parent_dir / "vscode-temp-user")
        exts_dir = str(parent_dir / "vscode-temp-exts")
        target_file = str(parent_dir / "usage.json")
        ...
        _browser = await _playwright.chromium.connect_over_cdp("http://localhost:9223")
        
        for attempt in range(10):
            target_page = await find_vscode_page(_browser)
            if target_page:
                _page = target_page
                screenshot = await _save_screenshot_and_log("electron_connect", f"Connected to launched Electron app page: {await _page.title()} on port 9223")
                return {
                    "status": "success",
                    "title": await _page.title(),
                    "url": _page.url,
        ...
        errors.append("Launched VS Code on port 9223, but could not find target page in Chromium contexts.")
    except Exception as e:
        errors.append(f"Failed to launch VS Code on port 9223: {e}")
        
    return {"status": "error", "message": "; ".join(errors)}

@mcp.tool()
async def browser_get_accessibility_tree() -> dict:
    """Get the accessibility tree (AX tree) of the current browser page. This allows Layer 2b 
    agents to inspect accessibility nodes, roles, and values of the page.
    """
    page = await _ensure_browser()
    try:
        ...
        return {"status": "error", "message": f"Failed to get accessibility tree: {e}"}
