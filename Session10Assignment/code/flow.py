...
class Executor:
    ...
    async def run(self, query: str, *, session_id: str | None = None,
        ...
        # Ensure session directory exists and start recording
        session_dir = store.dir
        video_path = session_dir / "recording.mp4"
        print(f"[flow] Starting screen recording to {video_path}")
        from recorder import ScreenRecorder
        recorder = ScreenRecorder(video_path, fps=5)
        recorder.start()
        ...
        try:
            ...
            print("[flow] Stopping screen recording...")
            recorder.stop()
            recorder.join()
            print("[flow] Screen recording saved.")
    ...
    async def _run_one(self, nid: str, graph: Graph, sid: str, query: str,
        ...
        try:
            # 5-minute timeout to guarantee that the orchestrator will not hang indefinitely if a tool/LLM hangs
            result, prompt = await asyncio.wait_for(
                run_skill(skill, nid, graph.g.nodes, sid, query, fr, memory_hits=memory_hits),
                timeout=300.0
            )
