...
class GeminiProvider(BaseProvider):
    ...
    async def chat(self, messages, *, max_tokens=2048, temperature=0.7, model=None,
        ...
        models_to_try = [m]
        backups = ["gemini-2.5-flash-lite", "gemini-flash-lite-latest", "gemini-3.5-flash"]
        for b in backups:
        ...
        for try_model in models_to_try:
            url = f"{self.base_url}/models/{try_model}:generateContent?key={self.api_key}"
            async with httpx.AsyncClient(timeout=180) as c:
                try:
                    ...
                    if isinstance(e, ProviderError) and e.status != 429:
                        raise e
                    last_error = e
                    continue
        else:
            if last_error:
                raise last_error
            raise ProviderError("All Gemini models failed")
