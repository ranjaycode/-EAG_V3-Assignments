...
class LLM:
    ...
    def chat(self, prompt: str = None, *,
        ...
        # Retry up to 6 times on 429/503 with a short backoff since gateway now
        # recovers in ~10s after RPM quota resets.
        wait = 15
        for attempt in range(6):
            r = httpx.post(f"{self.base_url}/v1/chat", json=body, timeout=self.timeout)
            if r.status_code not in (429, 503) or attempt == 5:
                r.raise_for_status()
                return r.json()
            print(f"[client] Rate limited ({r.status_code}) — waiting {wait}s for quota reset (retry {attempt + 2}/6) …")
            time.sleep(wait)
