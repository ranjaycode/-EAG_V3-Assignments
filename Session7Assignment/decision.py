...
_TOOL_DOCS: dict[str, str] = {
    ...
    "retrieve_cosmic_docs": (
        'retrieve_cosmic_docs(query: str, limit: int=3) '
        '→ list[{id, title, category, content}]  — search the Cosmic platform internal database containing articles, metrics, incident reports, APIs, market makers (e.g. MM001, INTMM1), and hedging engine runbooks'
    ),
    "index_paper": (
        'index_paper(path: str) '
        '→ {ok: bool, chunks_indexed: int}  — index a single markdown paper under papers/ directory into the FAISS vector database'
    ),
    "index_all_papers": (
        'index_all_papers(directory: str="papers") '
        '→ {ok: bool, chunks_indexed: int}  — index all markdown files recursively under papers/ directory into the FAISS vector database'
    ),
    "query_papers_index": (
        'query_papers_index(query: str, limit: int=3) '
        '→ list[{filename, section, content, score}]  — search the academic papers index semantically using vector embeddings (for Attention, ReAct, or Chain-of-Thought papers queries)'
    ),
}
