...
class DashboardHandler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
                ...
                for item in os.listdir(sessions_dir):
                    ...
                    try:
                        if os.path.isdir(item_path):
                            ...
                            # Clean up all other files, preserving .deleted to handle locks on Windows
        ...
        if self.path.startswith("/api/sessions/delete/"):
            sid = self.path.split("/")[-1]
            sessions_dir = os.path.join("state", "sessions")
            item_path = os.path.join(sessions_dir, sid)
            if os.path.exists(item_path) and os.path.isdir(item_path):
                ...
                # Clean up other files/dirs preserving .deleted
                for root, dirs, files in os.walk(item_path, topdown=False):
                    for name in files:
                        if name != ".deleted":
                            try:
    ...
    def handle_get_session(self, sid):
        ...
        try:
            ...
            cost_data = LLM().cost_by_agent(session=sid)
        except Exception as e:
            print("Error getting cost from gateway:", e)
