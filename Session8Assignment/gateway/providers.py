        ...
        async with httpx.AsyncClient(timeout=180) as c:
            for attempt in range(15):
                ...
                if r.status_code == 429:
                    sleep_time = 12.0 + 4.0 * attempt + random.uniform(1.0, 4.0)
                    print(f"[gateway.gemini] HTTP 429 rate limit hit. Retrying in {sleep_time:.2f}s... (attempt {attempt + 1}/15)")
                ...
                if r.status_code == 400:
                    ...
                    if r.status_code == 429:
                        sleep_time = 12.0 + 4.0 * attempt + random.uniform(1.0, 4.0)
                        print(f"[gateway.gemini] HTTP 429 rate limit hit on retry. Retrying in {sleep_time:.2f}s... (attempt {attempt + 1}/15)")
                        await asyncio.sleep(sleep_time)
                        continue
                break
